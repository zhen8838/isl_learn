...
def run(n: int, worker_count: int) -> None:
    ...
    prefix = root / "demo" / "build" / "row_sum_static_queue_demo"
    graph = row_sum_demo.get_task_graph(n)
    scheduled = static_queue_codegen.static_schedule(graph, worker_count)
    lib = static_queue_codegen.build(scheduled, prefix)
    ...
    print(f"static_queue max_abs_diff={max_abs_diff:.6f}")
    if max_abs_diff > 1e-4:
        raise RuntimeError("static queue verification failed")
    print("PASS")
